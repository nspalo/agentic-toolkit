...
### 2. Project Codebase (targeted search — now you know what to look for)
...
### 3. Other Projects in the Workspace

If neither the library nor the current project's code covers it, check sibling projects — their research docs, KB articles, investigation reports. Another project may have encountered and documented the same concept.
...
When any step reveals general knowledge (not project-specific), note it for potential promotion to `domain-knowledge/`. Always confirm with the human before promoting.

- Project-specific findings → `projects/{name}/knowledge-base/`
- General findings (useful across any project) → propose promotion to `domain-knowledge/` (with human approval)

## Why This Order

| Step | Cost | What you get |
|------|------|-------------|
| Library | Seconds — read 1 markdown file | Map: what it is, where it lives, which repo |
